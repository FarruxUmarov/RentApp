                    ...
                    <ul class="py-6 flex items-center list-none">
                        <li class="flex items-center lg:me-6 me-4">
                            <i data-feather="map-pin"
                               class="uil uil-compress-arrows lg:text-3xl text-2xl me-2 text-green-600"></i>
                            <span class="lg:text-xl">{{$ad->branch->name}}</span>
                        </li>

                        <li class="flex items-center lg:me-6 me-4">
                            <i data-feather="code"
                               class="uil uil-bed-double lg:text-3xl text-2xl me-2 text-green-600"></i>
                        ...
                        <div class="rounded-md bg-slate-50 dark:bg-slate-800 shadow dark:shadow-gray-700">
                            <div class="p-6">
                                ...
                                <div class="flex justify-between items-center mt-4">
                                    ...
                                    <span
                                        class="bg-green-600/10 text-green-600 text-sm px-2.5 py-0.75 rounded h-6">{{$ad->status->name }}</span>
                                </div>
...
{{--                                @if (Auth::user()->id == $ad->user_id)--}}
                                <form action="{{ route('ads.destroy', $ad->id) }}" method="POST"
                                    ...
                                    @method('DELETE')
                                    <button type="submit"
                                            class="btn bg-transparent hover:bg-red-600 border border-green-600 text-green-600 hover:text-white rounded-md">
